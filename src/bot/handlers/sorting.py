...
from src.services.operation import get_suggestion_for_unsorted_payment, get_unsorted_payment_by_tg_id, sort_operation
...
async def sorting_menu(message: Message, state: FSMContext) -> None:
    ...
    if message.from_user is None:
        return
    tg_id = (
        (await state.get_data()).get("tg_id")
        if local_state == BotPaymentsSorting.category_suggestion
    ...
    if unsorted_payment:
        ...
        suggested_category = await get_suggestion_for_unsorted_payment(unsorted_payment.shop, list(categories))
        ...
        category = await get_category_object(suggested_category, user.id)
        ...
        await state.update_data(
            category_id=category.id, unsorted_payment_id=unsorted_payment.id, shop=unsorted_payment.shop, tg_id=tg_id
        )
...
async def accept_suggested_category(message: Message, state: FSMContext) -> None:
    if message.from_user is None:
        return
...
async def reject_suggested_category(message: Message) -> None:
    if message.from_user is None:
        return
    await message.answer(
        "Please choose a category for the payment:",
...
async def handle_category_selection(callback_query: CallbackQuery, state: FSMContext) -> None:
